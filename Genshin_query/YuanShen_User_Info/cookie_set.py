...
class MoHoYoCookie:
    ...
    def check_limit(self, cid=-1, cookie=None, to_limit=False, remove=False):
        ...
        if to_limit:
            # cursor.execute(f"CREATE TABLE IF NOT EXISTS '{day}' ('id' INTEGER NOT NULL,'times' integer,"
            #                f"'limit' integer,PRIMARY KEY ('id'));")
            # cursor.execute(f"INSERT INTO limit (id, limit) VALUES (?, ?)", [cid, 1])
...
# print(MoHoYoCookie().check_limit(2, remove=True))
# print(MoHoYoCookie().get_cookie_list())
